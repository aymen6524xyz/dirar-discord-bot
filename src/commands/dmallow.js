...
module.exports = {
  ...
  perms: 5, // VIP check overrides this in code
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('time')
        .setDescription('Duration (e.g., 10m, 1h). Leave empty for permanent.')
        .setRequired(false)),
  ...
  async execute(message, args) {
    ...
    const timeStr = args[1]; // Optional time argument

    if (!userId) return message.reply("❗ Usage: `d?dmallow <user> [time]`\nExample: `d?dmallow @User 10m`");
    ...
    try {
        ...
        addAllowedUser(user.id, duration); // Always returns true or updates
        message.reply(`✅ **${user.tag}** is now allowed to use the DM Chat ${timeLabel}.`);
    } catch (e) {
        console.log(e);
  ...
  async executeSlash(interaction) {
    ...
    if (timeStr) {
        ...
        if (match) {
            ...
            return interaction.reply({ content: "❌ Invalid time format. Use `10m`, `1h`, `1d` etc.", ephemeral: true });
        }
    }

    addAllowedUser(user.id, duration);
    interaction.reply(`✅ **${user.tag}** is now allowed to use the DM Chat ${timeLabel}.`);
  }
};
